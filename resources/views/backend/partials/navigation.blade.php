        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="flex items-center text-sm font-medium mr-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
                            <div><span
                                    class="flag-icon flag-icon-{{config()->get('language')[app()->getLocale()]['flag-icon']}} mr-1"></span> {{ config()->get('language')[app()->getLocale()]['display'] }}
                            </div>
                        ...
                        @foreach (config()->get('language') as $lang => $language)
                            @if ($lang != app()->getLocale())
                                <x-dropdown-link :href="route('lang.switch', $lang)">
                                    <span
                                        class="flag-icon flag-icon-{{$language['flag-icon']}} mr-1"></span> {{$language['display']}}
                                </x-dropdown-link>
                            @endif
                        @endforeach
                    </x-slot>
